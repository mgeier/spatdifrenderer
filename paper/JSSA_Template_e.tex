...
\twoauthors
  ...
  {Jan C. Schacher} {Zurich University of the Arts\\Institute for Computer Music and Sound Technology \\ jan.schacher@zhdk.ch}
...
In this article we present the development of a tool \todo{tool? maybe better would be 'reference software' or 'reference implementation'} for easy integration of SpatDIF into existing software.
...
SpatDIF has become a lightweight, yet extendable, and easy integratable spatial scene description.
To make this lightweight description functional SpatDIF is equipped with many auxiliary features, for example the capability of dealing with temporal interpolation of scene descriptors as further described in the 2013 Computer Music Journal article \cite{Peters:2013SpatDifCMJ}.
%In contrast to the complex and MPEG specification \cite{scheirer1999audiobifs} 
...
%The responses in these meetings suggested the urgent need for 
\section{conceptual explanation / structure} %assigned to Jasch/Trond/Nils


One of the guiding principles for SpatDIF is the idea that authoring and rendering of spatial sound usually occurs at completely separate times and places, and that the rendering may use processing tools or reproduction setups whose capabilities cannot be known in advance. 
SpatDIF formulates a concise semantic structure that is capable of carrying the necessary information, without being tied to a specific implementation, thought-model or technical method. \todo{What is meant by thought-model?}
...
SpatDIF is meant to describe only the aspects required for the storage and transmission of \emph{spatial information}.
 and leaves out non-spatial properties of the content.
 


A basic set of SpatDIF functionalities are integrated in the SpatDIF core. 
The core provides the most essential, yet extensible set of functionalities for describing spatial sound scenes.
Additional functionalities are organized in extensions and can be added to the core if desired.
...
\caption{Class Structure \todo{the text in the Class Structure looks pixely, can we save it a vector graphics?}}
\label{fig:class_structure}
\end{figure*}

\subsubsection{sdScene}
sdScene maintains all data associated to a SpatDIF scene. This class offers clients the following three functionalities.
...
These functions utilize two external libraries, TinyXML-2 \footnote{http://www.grinninglizard.com/tinyxml/ accessed Oct. 9. 2013} and libjson \footnote{http://sourceforge.net/projects/libjson/ accessed Oct. 9. 2013}.

\subsubsection{sdEvent}
This is a pure abstract class of event, that maintains following three data.
...
The above mentioned basic class hierarchy is already implemented in the library. The library is also able to interpret simple XML, JSON and OSC messages and currently examined against SpatDIFRenderer, in order to further improve its performance.  All extensions defined in the SpatDIF specification 0.3 will be implemented in early 2014 and C interface will be added in the late 2014. The library will be completed by the end of 2014 and released under the MIT or similar license.
