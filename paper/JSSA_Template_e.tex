...
The following listing shows how to load an XML-formatted string into an sdScene and query the entity called `insect' for the first occurrence of an event which contains a position- and a media-descriptor.
...
\noindent Line 1: sdLoader::sceneFrom static function loads a SpatDIF scene from a XML formatted string.

\noindent Line 2: A pointer to an entity, named `insect' is obtained by scene.getEntity function.

\noindent Lines 3-4: The entity `insect' is requested to return a pointer to the first event with the position and media location descriptor.

\noindent Line 6: Querying the `insect' entity for its name

\noindent Lines 7-9: Posting the values and time of events to the console
...
One one of these extension-libraries that is being used for this application is libsndfile\footnote{http://www.mega-nerd.com/libsndfile/ URI valid 2013/10/23}, which is a powerful audiofile handling toolset.
...
Of course, an advance access to the code can be granted on demand.
